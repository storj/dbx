...
package single_column_select_paged
...
func (obj *sqlite3Impl) Paged_Foo_Pk(ctx context.Context,
	limit int, start *Paged_Foo_Pk_Continuation) (
	rows []*Pk_Row, next *Paged_Foo_Pk_Continuation, err error) {

	var __embed_stmt = __sqlbundle_Literal("SELECT foos.pk, foos.pk FROM foos WHERE (foos.pk) > ? ORDER BY foos.pk LIMIT ?")

	var __embed_first_stmt = __sqlbundle_Literal("SELECT foos.pk, foos.pk FROM foos ORDER BY foos.pk LIMIT ?")
	...
	for {
		rows, next, err = func() (rows []*Pk_Row, next *Paged_Foo_Pk_Continuation, err error) {
			...
			for __rows.Next() {
				row := &Pk_Row{}
				err = __rows.Scan(&row.Pk, &__continuation._value_pk)
				if err != nil {
					return nil, nil, err
				}
...
func (obj *pgxImpl) Paged_Foo_Pk(ctx context.Context,
	limit int, start *Paged_Foo_Pk_Continuation) (
	rows []*Pk_Row, next *Paged_Foo_Pk_Continuation, err error) {
...
func (obj *pgxcockroachImpl) Paged_Foo_Pk(ctx context.Context,
	limit int, start *Paged_Foo_Pk_Continuation) (
	rows []*Pk_Row, next *Paged_Foo_Pk_Continuation, err error) {
...
func (obj *spannerImpl) Paged_Foo_Pk(ctx context.Context,
	limit int, start *Paged_Foo_Pk_Continuation) (
	rows []*Pk_Row, next *Paged_Foo_Pk_Continuation, err error) {
...
type Methods interface {
	Paged_Foo_Pk(ctx context.Context,
		limit int, start *Paged_Foo_Pk_Continuation) (
		rows []*Pk_Row, next *Paged_Foo_Pk_Continuation, err error)
}
