...
type Foo_First_Field struct {
	_set   bool
	_null  bool
	_value int
}

func Foo_First(v int) Foo_First_Field {
	return Foo_First_Field{_set: true, _value: v}
}

func (f Foo_First_Field) value() any {
	if !f._set || f._null {
		return nil
	}
	return f._value
...
type Foo_Second_Field struct {
	_set   bool
	_null  bool
	_value int
}

func Foo_Second(v int) Foo_Second_Field {
	return Foo_Second_Field{_set: true, _value: v}
}

func (f Foo_Second_Field) value() any {
	if !f._set || f._null {
		return nil
	}
	return f._value
...
func (obj *sqlite3Impl) All_Foo_OrderBy_Desc_First_Asc_Second(ctx context.Context) (
	rows []*Foo, err error) {

	var __embed_stmt = __sqlbundle_Literal("SELECT foos.pk, foos.first, foos.second FROM foos ORDER BY foos.first DESC, foos.second")
	...
	for {
		rows, err = func() (rows []*Foo, err error) {
			...
			for __rows.Next() {
				foo := &Foo{}
				err = __rows.Scan(&foo.Pk, &foo.First, &foo.Second)
				if err != nil {
					return nil, err
				}
...
func (obj *pgxImpl) All_Foo_OrderBy_Desc_First_Asc_Second(ctx context.Context) (
	rows []*Foo, err error) {
...
func (obj *pgxcockroachImpl) All_Foo_OrderBy_Desc_First_Asc_Second(ctx context.Context) (
	rows []*Foo, err error) {
...
func (obj *spannerImpl) All_Foo_OrderBy_Desc_First_Asc_Second(ctx context.Context) (
	rows []*Foo, err error) {
...
type Methods interface {
	All_Foo_OrderBy_Desc_First_Asc_Second(ctx context.Context) (
		rows []*Foo, err error)
}
