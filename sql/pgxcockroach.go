...
func (p *pgxcockroach) RowId() string {
	return ""
}

func (p *pgxcockroach) ColumnType(field *ir.Field) string {
	switch field.Type {
...
func (p *pgxcockroach) ArgumentPrefix() string { return "$" }

var pgxCockroachEscaper = strings.NewReplacer(
	`'`, `''`,
)

func (p *pgxcockroach) EscapeString(s string) string {
	return pgxCockroachEscaper.Replace(s)
}

func (p *pgxcockroach) BoolLit(v bool) string {
	if v {
		return "true"
	}
	return "false"
...
func (p *pgxcockroach) DefaultLit(v string) string {
